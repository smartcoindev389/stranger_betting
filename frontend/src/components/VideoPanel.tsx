          ...
          if (socket) {
            ...
            if (!rtcManagerRef.current) {
              ...
              try {
                ...
                setIsConnecting(false);
                showNotification(t('video.failedToEstablish'), 'error');
              }
            } else {
              ...
              if (localStream) {
                try {
                  ...
                  setIsConnecting(false);
                  showNotification(t('video.failedToUpdate'), 'error');
                }
              }
            }
          }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-gray-800">{t('video.title')}</h3>
        {isConnecting && (
          <span className="text-xs text-blue-600">{t('video.connecting')}</span>
        )}
      </div>

      <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4 min-h-0">
        {/* Local Video */}
        <div className="aspect-video bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl flex items-center justify-center relative overflow-hidden w-full">
          <video 
            ref={localVideoRef}
          ...
          {!isVideoOn && (
            <div className="absolute inset-0 flex items-center justify-center text-center">
              <div>
                ...
                <p className="text-gray-400 text-xs">{t('common.you')}</p>
              </div>
            </div>
          )}
      ...
      <div className="flex gap-2 justify-center">
        <button
          ...
          title={isVideoOn ? t('video.turnOffVideo') : t('video.turnOnVideo')}
        >
